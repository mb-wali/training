...
Now we need to register the signal in our Invenio instance. We have to connect
the signal with our indexer at `ext.py` in the `init_app` of our extension.
...
We can now create a new record, using the deposit of the previous exercise, and verify in Elasticsearch <http://127.0.0.1:9200/records/_search?pretty=true> the modified fields.
